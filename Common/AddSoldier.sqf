...
if ( (_si == si0 || _si == si1) && _gi == -1 && isNull _group ) then { call format["_groupCreate = groupTemp%1", _si]; _joinNull = true };
if ( (_si == si0 || _si == si1) && _gi != -1 ) then { _groupCreate = (groupMatrix select _si) select _gi };
...
if (!_err) then
{
  ...
  _value = _type + 200*_gi + 200*18*_giBuyer + 200*18*18*_si;
